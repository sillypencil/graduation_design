...
//新增任务
 function insertTask(client , obj, callback){
     client.query('insert into task values(?,?,?,?,?,?,?,?)', 
        [obj.tnumber, obj.name, obj.content, obj.days, obj.personnum, obj.finish, obj.startdate,obj.pnumber], 
        function(err,result){
         if( err ){
...
function updateTask(client , obj , callback){
    client.query('update task set name =? ,content =? ,days =? ,personnum =? ,finish =? ,startdate =? ,pnumber =? where tnumber ="' + obj.tnumber + '"',
...
function  selectTnum(client,pnumber,callback){
     client.query('select name from task where pnumber="'+pnumber+'"',function(err,results,fields){
         if(err) throw err;
